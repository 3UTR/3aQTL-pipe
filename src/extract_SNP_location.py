...
usage: python extract_SNP_location.py --genotype_bed /path/to/genotype_matrix.bed --output /path/to/output/snp_location.txt
'''
import argparse
...
for line in fh.readlines():
    ...
    if len(w)>=2:
        ...
        print("%s\t%s\t%s" % (snp,chrom,pos), file=fho)
    else:
        print("Error:",snp)
fh.close()
fho.close()
