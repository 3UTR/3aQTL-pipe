...
function run_fine_mapping(){
	...
	if [ $threads -eq 1 ]
	then
		for gene in `cat ${currDir}/FineMapping/input/picked_asso.loc_${w}.txt|cut -f1`
		do
			echo "Analyzing $gene"
		...
		split -l $threads -d ${currDir}/FineMapping/input/picked_asso_list.loc_${w}.txt ${currDir}/FineMapping/output/tmp/finemap_task_ &
		wait
